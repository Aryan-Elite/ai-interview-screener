...
type Vote = { adminEmail: string; vote: "move_forward" | "hold"; votedAt: string };
...
export default function CandidateDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function handleVote(vote: "move_forward" | "hold") {
    setVoting(true);
  ...
  const holdCount = assessment.votes.filter(v => v.vote === "hold").length;
      ...
      <main className="flex-1 ml-60 p-8">
        ...
        <div className="space-y-5">
          ...
          <Card className="shadow-sm bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-800">
            ...
            <CardContent className="px-6 pb-6">
              ...
              <div className="flex gap-3 mb-4">
                <Button variant="outline" onClick={() => handleVote("move_forward")} disabled={voting}
                ...
                <Button variant="outline" onClick={() => handleVote("hold")} disabled={voting}
                  className={`flex-1 text-base h-11 ${myVote === "hold"
                    ? "border-amber-500 bg-amber-50 dark:bg-amber-950/30 text-amber-700 dark:text-amber-400 hover:bg-amber-50 dark:hover:bg-amber-950/30"
                    : "text-gray-600 dark:text-gray-400 border-gray-200 dark:border-gray-700"}`}>
                  ...
                  {assessment.votes.map(v => (
                    <p key={v.adminEmail} className="text-sm text-gray-400 dark:text-gray-500">
                      ...
                      <span className={v.vote === "move_forward"
                        ...
                        {v.vote === "move_forward" ? "Move Forward" : "Hold"}
                      </span>
                    </p>
                  ))}
                </div>
          ...
          {(() => {
            ...
            return (
              <Card className="shadow-sm border-amber-200 dark:border-amber-800 bg-white dark:bg-gray-900">
                <CardContent className="px-6 py-5 flex items-center justify-between">
                  <div>
                    <p className="text-base font-semibold text-amber-600 dark:text-amber-500">
                  ...
                  <span className="text-3xl font-bold text-amber-500">{count}</span>
                </CardContent>
              </Card>
            );
          })()}
